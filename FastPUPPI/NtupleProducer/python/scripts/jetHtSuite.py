...
def makeCorrArray(tree, what, obj, ptCorrCut, etaCut, corr, _cache={}):
    ...
    ret = ROOT.makeJetArray(tree, obj, ptCorrCut, etaCut, cppcalc, corr);    # the line which errors
    _cache[_key] = ret
...
whats = WHATS + [
    ('l1pfpu_hscseeds',[
        ("HSC 1x1 Seed",  "histo9x9SCPuppiCorr$",    ROOT.kGreen+3, 34, 1.1),
        ("HSC 3x3 Seed",  "histo9x9SCPuppi3x3$",    ROOT.kRed+3, 34, 1.1),
        ("HSC 5x5 Seed",  "histo9x9SCPuppi5x5$",    ROOT.kOrange+3, 34, 1.1),
        ("HSC 7x7 Seed",  "histo9x9SCPuppi7x7$",    ROOT.kBlue+3, 34, 1.1),
        ("HSC 9x9 Seed",  "histo9x9SCPuppi9x9$",    ROOT.kYellow+3, 34, 1.1)
    ]),
    ('l1pfpu_hscJets',[
        ("SC", "scPuppiCorr$", ROOT.kOrange+3, 34, 1.1),
        ("HSC 9x9",  "histoSCPuppiCorr$",    ROOT.kGreen+3, 34, 1.1),
        ("HSC 7x7",  "histo7x7SCPuppiCorr$",    ROOT.kBlue+3, 34, 1.1)
    ]),
    ('oldcomp',[
...
signal, background = [ f.Get("Events") for f in tfiles ]    # background is 
...
for plotkind in options.plots.split(","):    # Iterates over the plots to be made given by the user
  progress = _progress("Make plot %s: \n" % plotkind)    # Prints which plot is being made
  if plotkind not in ("rate","inclrate"):    # If the plot is not a rate plot
    if isJetMet:    # If the plot is a jet/met plot
        genArray = makeGenArray(signal, what, options.pt, options.eta)
    else:
        genArray = False; genObjName = None
        for objset,things in whats:     # objset is the name of the set of objects (e.g. l1pf, l1pfpu, etc.), things is the list of objects in the set
      ...
      for objset,things in whats:
          ...
          if options.what_reg:
              if not any(re.match(p+"$",objset) for p in options.what_reg.split(",")): 
                  continue
          plots = []
          for name,obj,col,msty,msiz in things:    # obj is the name of the object (e.g. L1Puppi, L1PuppiTk, etc.)
              obj = obj.rstrip("$")
              ...
              if plotkind == "rate":
                  if isJetMet:
                      print("Background: {}\nWhat: {}\nObject: {}\nPtcut: {}\nEta: {}\nJecs: {}".format(background, what, obj, ptcut, options.eta, jecs))
                      recoArrayB = makeCorrArray(background, what, obj, ptcut, options.eta, jecs) # background is the background tree, what is the variable (e.g. ht, met, etc.), obj is the object (e.g. L1Puppi, L1PuppiTk, etc.), ptcut is the pt cut, options.eta is the eta cut, jecs is the JECs
                  else:
                      recoArrayB = makeRecoLepArray(background, what, obj, ptcut, options.etas, lepid=options.lepid_, lepiso=options.lepiso_) 
